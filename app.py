...
# 3. BASE DE DADOS COM CACHE (Protegida contra erros de tipo com métricas)
@st.cache_data
def carregar_dados():
    ...
    for col in ['SJR', 'JIF', 'h-index', 'H index']:
        if col in df.columns:
            ...
            # Guardamos os valores estritamente convertidos em nulos oficiais temporariamente
            df[col] = pd.to_numeric(df[col], errors='coerce')
            
    # --- SUBSTITUIÇÃO RÁPIDA E VETORIZADA POR '-' ---
    # Substitui os valores nulos oficiais do Python (NaN) e textos "None" genéricos
    df = df.fillna("-")
...
with col_m2: 
    # Converte temporariamente para numérico forçando o traço '-' a virar NaN apenas para o max()
