    ...
    if disparar_busca:
        if not titulo_artigo or not resumo_artigo:
            ...
            if cache_key in st.session_state.ia_cache:
                ...
                # Motor principal de IA: recomendar_regras.py (MiniLM + A a Z completo)
                journals = None
                    ...
                    try:
                        ...
                        for _, row in df_rec.iterrows():
                            ...
                            journals.append({
                                ...
                                "indexador": indexador,
                                "jif": str(row.get("jif", "-")),
                                "quartil_jcr": str(row.get("quartil_jcr", "-")),
                                "sjr": str(row.get("sjr", "-")),
                                "sjr_quartile": str(row.get("sjr_quartile", "-")),
                                "h_index": str(row.get("h_index", "-")),
                                "h5_index": "-",
                                "h5_median": "-",
