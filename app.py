...
try:
    ...
    # Tratamento explícito da coluna de Homepage
...
# 4. ESTRUTURA DO MENU LATERAL (SISTEMA INTEGRADO)
st.sidebar.markdown(f"""
    <div style='display: flex; align-items: center; gap: 12px; margin-bottom: 20px;'>
        <h2 style='margin: 0; font-size: 1.60rem; font-weight: 700; color: #0F172A;'>{t['nav_tit']}</h2>
    </div>
...
# --- 9. INTERFACE PRINCIPAL MULTI-ABAS ---
...
with tab_ia:
    ...
    with col_input:
        titulo_artigo = st.text_input(t['ia_campo_titulo'], placeholder="Ex: Análise Epidemiológica de Saúde Coletiva...")
        resumo_artigo = st.text_area(t['ia_campo_resumo'], placeholder="Paste or type abstract here...", height=250)
    ...
    if st.button(t['ia_btn_buscar'], type="primary"):
        if not HAS_GEMINI:
            ...
            with st.spinner(t['ia_analisando']):
                ...
                # Se houverem muitos registros, enviamos até 100 ao contexto da IA
                # para que ela possa selecionar e ordenar de forma qualificada até as 10 melhores
                ...
                try:
                    ...
                    for rec in recomendacoes:
                        ...
                        # Renderização de card para cada recomendação (até 10 dinâmicas)
